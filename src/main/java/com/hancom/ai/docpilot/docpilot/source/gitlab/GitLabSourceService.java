...
public class GitLabSourceService {
    ...
    public GitLabSourceService(SystemSettingService settingService) {
        this.settingService = settingService;
    }

    private GitLabApi getGitLabApi() {
        String currentHash = buildConfigHash();
        if (gitLabApi == null || !currentHash.equals(lastConfigHash)) {
            synchronized (this) {
                currentHash = buildConfigHash();
                if (gitLabApi == null || !currentHash.equals(lastConfigHash)) {
                    String url = settingService.get(SystemSettingService.GITLAB_URL);
                    String token = settingService.get(SystemSettingService.GITLAB_TOKEN);
                    this.gitLabApi = new GitLabApi(url, token);
                    this.lastConfigHash = currentHash;
                    log.info("GitLab API 재초기화 완료: {}", url);
                }
            }
        }
        return gitLabApi;
    ...
    private String buildConfigHash() {
        return settingService.get(SystemSettingService.GITLAB_URL) + "|"
                + settingService.get(SystemSettingService.GITLAB_TOKEN);
    }

    /**
     * 설정 변경 후 GitLabApi를 강제로 재생성합니다.
     */
    public void resetGitLabApi() {
        synchronized (this) {
            this.gitLabApi = null;
    ...
    public List<String> getChangedFiles(Long projectId, String commitSha) throws GitLabApiException {
        List<Diff> diffs = getGitLabApi().getCommitsApi().getDiff(projectId, commitSha);
    ...
    public String getFileContent(Long projectId, String filePath, String ref) throws GitLabApiException {
        RepositoryFile file = getGitLabApi().getRepositoryFileApi().getFile(projectId, filePath, ref);
    ...
    public String getDefaultBranch(Long projectId) throws GitLabApiException {
        Project project = getGitLabApi().getProjectApi().getProject(projectId);
    ...
    public List<String> findFiles(Long projectId, String ref, String nameContains) throws GitLabApiException {
        List<TreeItem> items = getGitLabApi().getRepositoryApi().getTree(projectId, null, ref, true);
        return items.stream()
    ...
    public String getCommitMessage(Long projectId, String commitSha) throws GitLabApiException {
        Commit commit = getGitLabApi().getCommitsApi().getCommit(projectId, commitSha);
